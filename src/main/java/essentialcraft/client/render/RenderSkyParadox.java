...
public class RenderSkyParadox  extends IRenderHandler {
	...
	public void render(float partialTicks, WorldClient world, Minecraft mc) {
		...
		BufferBuilder.pos(-f10, 100.0D, -f10).tex(0.0D, 0.0D).endVertex();
		BufferBuilder.pos(f10, 100.0D, -f10).tex(1.0D, 0.0D).endVertex();
		BufferBuilder.pos(f10, 100.0D, f10).tex(1.0D, 1.0D).endVertex();
		BufferBuilder.pos(-f10, 100.0D, f10).tex(0.0D, 1.0D).endVertex();
		tessellator1.draw();
		f10 = 20.0F;
		...
		float f17 = (i1 + 1) / 2.0F;
		BufferBuilder.begin(7, DefaultVertexFormats.POSITION_TEX);
		BufferBuilder.pos(-f10, -100.0D, f10).tex(f16, f17).endVertex();
		BufferBuilder.pos(f10, -100.0D, f10).tex(f14, f17).endVertex();
		BufferBuilder.pos(f10, -100.0D, -f10).tex(f14, f15).endVertex();
		BufferBuilder.pos(-f10, -100.0D, -f10).tex(f16, f15).endVertex();
		tessellator1.draw();
		GlStateManager.disableTexture2D();

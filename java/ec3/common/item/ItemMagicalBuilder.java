	...
	public ActionResult<ItemStack> onItemRightClick(ItemStack is, World w, EntityPlayer p, EnumHand hand)
	{
			...
			if(!p.isSneaking())
			{
				...
				if(!hasPos1)
				{
					...
					if(p.worldObj.isRemote)
						p.addChatMessage(new TextComponentString("[Magical Builder] First position: "+mop.getBlockPos().getX()+","+mop.getBlockPos().getY()+","+mop.getBlockPos().getZ()).setStyle(new Style().setColor(TextFormatting.DARK_PURPLE)));
				...
				if(!hasPos2)
				{
					...
					{
						if(p.worldObj.isRemote)
							p.addChatMessage(new TextComponentString("[Magical Builder]The distance between points ("+dist.getDistance()+") is too large, max: 48!").setStyle(new Style().setColor(TextFormatting.DARK_PURPLE)));
						return new ActionResult(EnumActionResult.PASS, is);
					}
					this.setSecondPoint(is, mop.getBlockPos().getX(), mop.getBlockPos().getY(), mop.getBlockPos().getZ());
					if(p.worldObj.isRemote)
						p.addChatMessage(new TextComponentString("[Magical Builder] Second position: "+mop.getBlockPos().getX()+","+mop.getBlockPos().getY()+","+mop.getBlockPos().getZ()).setStyle(new Style().setColor(TextFormatting.DARK_PURPLE)));
					return new ActionResult(EnumActionResult.PASS, is);
				}
				...
				{
					if(p.worldObj.isRemote)
						p.addChatMessage(new TextComponentString("[Magical Builder]Both points already set! Shift-rightclick air to reset!").setStyle(new Style().setColor(TextFormatting.DARK_PURPLE)));
				}
			}else
			{
				if(this.setStoredStack(is, w, mop.getBlockPos().getX(), mop.getBlockPos().getY(), mop.getBlockPos().getZ()))
				{
					...
					{
						if(p.worldObj.isRemote)
							p.addChatMessage(new TextComponentString("[Magical Builder]Set the block to: "+this.retrieveStackFromNBT(is).getDisplayName()).setStyle(new Style().setColor(TextFormatting.DARK_PURPLE)));
					}
				}
			}
		}else if(mop==null)
		{
			if(p.isSneaking())
			{
				...
				if(p.worldObj.isRemote)
					p.addChatMessage(new TextComponentString("[Magical Builder]Both points reseted!").setStyle(new Style().setColor(TextFormatting.DARK_PURPLE)));
			}else
			{
				if(this.hasFirstPoint(is) && this.hasSecondPoint(is) && ((this.hasStoredBlock(is) && this.retrieveStackFromNBT(is) != null) || is.getItemDamage() == 1))
				{
					...
					if(p.worldObj.isRemote)
						p.addChatMessage(new TextComponentString("[Magical Builder]Filled selected area! "+setted+" blocks got replaced!").setStyle(new Style().setColor(TextFormatting.DARK_PURPLE)));
				}
					...
					for(int z = 0; z <= diffZ; ++z)
					{
						...
						if(is.getItemDamage() == 0)
						{
							if(e.worldObj.isAirBlock(dp))
							{
								...
								if(ForgeHooks.onPlaceItemIntoWorld(settedTo, e, e.worldObj, dp, EnumFacing.DOWN, 0, 0, 0, EnumHand.MAIN_HAND) == EnumActionResult.SUCCESS)
									++itemsSet;
								else
								{
						...
						if(is.getItemDamage() == 1)
						{
							...
							if(e.worldObj.getBlockState(dp).getBlockHardness(e.worldObj, dp) >= 0 && !e.worldObj.isRemote)
							{
								...
								int be = ForgeHooks.onBlockBreakEvent(e.worldObj, type, (EntityPlayerMP)e, dp);
								if(be != -1)
								{
									e.worldObj.getBlockState(dp).getBlock().dropBlockAsItem(e.worldObj, dp, e.worldObj.getBlockState(new BlockPos(dx, dy, dz)), 0);
									e.worldObj.setBlockToAir(dp);
									++itemsSet;
								}
							}
						}
						if(is.getItemDamage() == 2)
						{
							...
							if(e.worldObj.getBlockState(dp).getBlockHardness(e.worldObj, dp) >= 0 && !e.worldObj.isRemote)
							{
								ItemStack worldStack = new ItemStack(e.worldObj.getBlockState(dp).getBlock(),1,e.worldObj.getBlockState(dp).getBlock().getMetaFromState(e.worldObj.getBlockState(dp)));
								if(worldStack != null && setTo.isItemEqual(worldStack))
								{
									...
									if(be != -1)
									{
										e.worldObj.getBlockState(dp).getBlock().dropBlockAsItem(e.worldObj, dp, e.worldObj.getBlockState(dp), 0);
										e.worldObj.setBlockToAir(dp);
										++itemsSet;
									}
								}
						...
						if(is.getItemDamage() == 4)
						{
							...
							}else
							{
								if(e.worldObj.getBlockState(dp).getBlockHardness(e.worldObj, dp) >= 0 && !e.worldObj.isRemote)
								{
									...
									{
										e.worldObj.getBlockState(dp).getBlock().dropBlockAsItem(e.worldObj, dp, e.worldObj.getBlockState(dp), 0);
										e.worldObj.setBlockState(dp, Block.getBlockFromItem(setTo.getItem()).getStateFromMeta(setTo.getItemDamage()), 3);
										++itemsSet;
									}

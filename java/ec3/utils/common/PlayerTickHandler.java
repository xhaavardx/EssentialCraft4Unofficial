...
public class PlayerTickHandler {
		...
		if(Keyboard.isKeyDown(Keyboard.KEY_R) && !isRKeyPressed)
		{
			...
			if(currentPlayerItem != null && currentPlayerItem.getItem() == ItemsCore.research_book)
			{
				...
				s:for(int o = 0; o < 4; ++o)
				{
					...
					List<EntityItem> lst = e.worldObj.getEntitiesWithinAABB(EntityItem.class, new AxisAlignedBB(e.posX+itemSearchVec.xCoord-0.5D, e.posY+itemSearchVec.yCoord-0.5D, e.posZ+itemSearchVec.zCoord-0.5D, e.posX+itemSearchVec.xCoord+0.5D, e.posY+itemSearchVec.yCoord+0.5D, e.posZ+itemSearchVec.zCoord+0.5D));
					if(!lst.isEmpty())
					{
				...
				if(!foundItem)
				{
					...
					s:for(int o = 0; o < 4; ++o) {
						...
						BlockPos pos = new BlockPos(MathHelper.floor_double(blockSearchVec.xCoord+e.posX), MathHelper.floor_double(blockSearchVec.yCoord+e.posY), MathHelper.floor_double(blockSearchVec.zCoord+e.posZ));
						Block blk = e.worldObj.getBlockState(pos).getBlock();
						if(blk != null && blk != Blocks.AIR)
						{
							ItemStack heldIS = new ItemStack(blk,1,blk.getMetaFromState(e.worldObj.getBlockState(pos)));
							if(ApiCore.findDiscoveryByIS(heldIS) != null)
							{
								DiscoveryEntry switchTo = ApiCore.findDiscoveryByIS(heldIS);
								GuiResearchBook.currentPage = 0;
	...
	public void client_manageWorldEvents(EntityPlayer e)
	{
		...
		{
			if(e.worldObj.provider.getDimension() == Config.dimensionID)
			{
				...
				if(ECUtils.isEventActive("ec3.event.darkness"))
				{
					if(e.worldObj.rand.nextFloat() < 0.01F)
						e.worldObj.playSound(e.posX,e.posY,e.posZ, SoundEvents.AMBIENT_CAVE,SoundCategory.AMBIENT, 1, e.worldObj.rand.nextFloat()*2, true);
					if(e.worldObj.rand.nextFloat() < 0.001F)
					{
						...
						e.worldObj.playSound(e.posX+MathUtils.randomDouble(e.worldObj.rand)*16,e.posY,e.posZ+MathUtils.randomDouble(e.worldObj.rand)*16, sound[e.worldObj.rand.nextInt(sound.length)], SoundCategory.HOSTILE, 1, 0.01F, true);
					}
				...
				if(ECUtils.isEventActive("ec3.event.earthquake"))
				{
					if(!e.capabilities.isCreativeMode) {
						...
						if(e.worldObj.rand.nextFloat() < 0.01F)
						{
							...
							e.worldObj.playSound(e.posX,e.posY,e.posZ, SoundEvents.ENTITY_GENERIC_EXPLODE, SoundCategory.BLOCKS, 0.1F, 0.1F, true);
						}
					}
				}
			}
				...
				if(e.ticksExisted % 1200 == 0) {
					...
					if(e.worldObj.rand.nextInt(36000) <= data.getOverhaulDamage()) {
						...
						if(!possibleEffects.isEmpty()) {
							...
							e.addChatMessage(new TextComponentString(I18n.translateToLocal("ec3.txt.corruption")).setStyle(new Style().setColor(TextFormatting.RED)));
							ECUtils.requestSync(e);
						}
					}
				}
	...
	public void doGroundItemChecks(EntityItem item) {
		if(item.getEntityItem().getItem() == Items.BLAZE_POWDER) {
			Block b = item.worldObj.getBlockState(new BlockPos(MathHelper.floor_double(item.posX), MathHelper.floor_double(item.posY)-1, MathHelper.floor_double(item.posZ))).getBlock();
			if(b == Blocks.NETHERRACK) {
				IBlockState b_check_b1 = item.worldObj.getBlockState(new BlockPos(MathHelper.floor_double(item.posX)+1, MathHelper.floor_double(item.posY)-1, MathHelper.floor_double(item.posZ)));
				IBlockState b_check_b2 = item.worldObj.getBlockState(new BlockPos(MathHelper.floor_double(item.posX)-1, MathHelper.floor_double(item.posY)-1, MathHelper.floor_double(item.posZ)));
				IBlockState b_check_b3 = item.worldObj.getBlockState(new BlockPos(MathHelper.floor_double(item.posX), MathHelper.floor_double(item.posY)-1, MathHelper.floor_double(item.posZ)+1));
				IBlockState b_check_b4 = item.worldObj.getBlockState(new BlockPos(MathHelper.floor_double(item.posX), MathHelper.floor_double(item.posY)-1, MathHelper.floor_double(item.posZ)-1));
				if(
						item.worldObj.isAirBlock(new BlockPos(MathHelper.floor_double(item.posX), (int)item.posY, MathHelper.floor_double(item.posZ))) &&
						...
						b_check_b3.getMaterial() == Material.LAVA)
				{
					...
					if(item.worldObj.rand.nextFloat() <= 0.8F) {
						item.worldObj.setBlockState(new BlockPos(MathHelper.floor_double(item.posX), (int)item.posY, MathHelper.floor_double(item.posZ)), BlocksCore.drops.getStateFromMeta(0), 3);
					}
				}
			}
		}
			...
			if(b == Blocks.ICE) {
				...
				IBlockState b_check_b3 = item.worldObj.getBlockState(new BlockPos(MathHelper.floor_double(item.posX), MathHelper.floor_double(item.posY)-1, MathHelper.floor_double(item.posZ)+1));
				IBlockState b_check_b4 = item.worldObj.getBlockState(new BlockPos(MathHelper.floor_double(item.posX), MathHelper.floor_double(item.posY)-1, MathHelper.floor_double(item.posZ)-1));
				if(item.worldObj.isAirBlock(new BlockPos(MathHelper.floor_double(item.posX), (int)item.posY, MathHelper.floor_double(item.posZ))) &&
						...
						b_check_b3.getMaterial() == Material.WATER)
				{
					...
					MiscUtils.spawnParticlesOnServer("explosion_normal", (float)item.posX, (float)item.posY, (float)item.posZ, 0D, 0D, 0D);
					if(item.worldObj.rand.nextFloat() <= 0.8F) {
						item.worldObj.setBlockState(new BlockPos(MathHelper.floor_double(item.posX), (int)item.posY, MathHelper.floor_double(item.posZ)), BlocksCore.drops.getStateFromMeta(1), 3);
					}
				}
			}
		}
			...
			if(b == Blocks.SLIME_BLOCK) {
						...
						b_check_b3.getBlock() == Blocks.GRASS)
				{
					...
					MiscUtils.spawnParticlesOnServer("explosion_normal", (float)item.posX, (float)item.posY, (float)item.posZ, 0D, 0D, 0D);
					if(item.worldObj.rand.nextFloat() <= 0.8F) {
						item.worldObj.setBlockState(new BlockPos(MathHelper.floor_double(item.posX), (int)item.posY, MathHelper.floor_double(item.posZ)), BlocksCore.drops.getStateFromMeta(2), 3);
					}
				}
			}
		}
			...
			if(b == Blocks.QUARTZ_BLOCK) {
						...
						b_check_b3.getBlock() == Blocks.SAND)
				{
					...
					MiscUtils.spawnParticlesOnServer("explosion_normal", (float)item.posX, (float)item.posY, (float)item.posZ, 0D, 0D, 0D);
					if(item.worldObj.rand.nextFloat() <= 0.8F) {
						item.worldObj.setBlockState(new BlockPos(MathHelper.floor_double(item.posX), (int)item.posY, MathHelper.floor_double(item.posZ)), BlocksCore.drops.getStateFromMeta(3), 3);
					}
				}
			}
		}

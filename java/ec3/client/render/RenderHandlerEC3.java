...
public class RenderHandlerEC3 {
	...
	public void renderParadox()
	{
		...
		if(paradoxID == 0)
		{
			...
			{
				mc.thePlayer.worldObj.playSound(mc.thePlayer.posX, mc.thePlayer.posY, mc.thePlayer.posZ, SoundEvents.AMBIENT_CAVE, SoundCategory.AMBIENT, 100, 0.01F, false);
			}
			if(currentParadoxTicks == 199)
			{
			...
			if(currentParadoxTicks >= 10)
				for(int i = 0; i < 20; ++i)
				{
					mc.theWorld.spawnParticle(EnumParticleTypes.REDSTONE, mc.thePlayer.posX+MathUtils.randomDouble(mc.theWorld.rand)*16, mc.thePlayer.posY+MathUtils.randomDouble(mc.theWorld.rand)*16, mc.thePlayer.posZ+MathUtils.randomDouble(mc.theWorld.rand)*16, -1, 0, 0);
				}
		}
		if(paradoxID == 1)
		{
		...
		if(paradoxID == 2)
		{
			...
			if(currentParadoxTicks < 190 && currentParadoxTicks > 10)
			{
				...
				{
					mc.theWorld.playSound(explosion.x, explosion.y, explosion.z, SoundEvents.ENTITY_GENERIC_EXPLODE, SoundCategory.BLOCKS, 3, 0.1F, true);
	...
	public void renderFog(EntityViewRenderEvent.RenderFogEvent evt) {
		...
		if(is != null) {
			if(is.getItem() instanceof ItemOrbitalRemote) {
				...
				RayTraceResult mop = p.worldObj.rayTraceBlocks(lookVec, distanced, true, false, false);
				...
				if(mop != null && mop.typeOfHit == Type.BLOCK) {
					...
					TileEntityBeaconRenderer.renderBeamSegment(0, 0, 0, evt.getRenderPartialTicks(), 1, p.worldObj.getTotalWorldTime(), 0, 255-mop.getBlockPos().getY(), new float[] {1,0,1}, 0.2F, 0.5F);
	...
	public void renderBlockOverlays(RenderWorldLastEvent evt) {
		...
		if(is != null) {
			if(is.getTagCompound() != null && is.getItem() instanceof ItemBoundGem && is.getTagCompound().hasKey("pos") && is.getTagCompound().getInteger("dim") == p.dimension) {
				...
				if(!p.worldObj.isBlockLoaded(new BlockPos((int)mx, (int)my, (int)mz)))
					return;

				AxisAlignedBB aabb = p.worldObj.getBlockState(new BlockPos((int)mx, (int)my, (int)mz)).getBlock().getSelectedBoundingBox(p.worldObj.getBlockState(new BlockPos((int)mx, (int)my, (int)mz)),p.worldObj, new BlockPos((int)mx, (int)my, (int)mz));
					...
					try {
						...
						for(int i = 0; i < gc.inventorySlots.inventorySlots.size(); ++i) {
							...
							if((slt.inventory instanceof TileEntity) || (slt.inventory instanceof InventoryBasic)) {
								...
								Minecraft.getMinecraft().fontRendererObj.drawString(""+slt.slotNumber, (k+slt.xDisplayPosition)*2, (l+slt.yDisplayPosition)*2, 0x000000);
								GlStateManager.popMatrix();
									...
									if(RenderHandlerEC3.slotsTable.containsKey(inv)) {
										...
										if(accessibleSlots.containsKey(slt.slotNumber)) {
											...
											if(lst != null && !lst.isEmpty()) {
												...
												GlStateManager.pushMatrix();
												GlStateManager.scale(0.5F, 0.5F, 0.5F);
												Minecraft.getMinecraft().fontRendererObj.drawString(""+d, (k+slt.xDisplayPosition)*2, (l+slt.yDisplayPosition+12)*2, 0x000000);
												GlStateManager.popMatrix();
											}
										}
									}
		...
		if(event.player.worldObj.isRemote && event.phase == Phase.START)
			if(event.player instanceof EntityPlayerSP) {
				...
				if(PotionRegistry.paradox != null && mc.thePlayer.getActivePotionEffect(PotionRegistry.paradox) != null) {
					...
					if(duration > 100) {
						if(duration % 100 == 0)
							mc.theWorld.playSound(mc.thePlayer.posX, mc.thePlayer.posY, mc.thePlayer.posZ, SoundRegistry.potionHeartbeat, SoundCategory.PLAYERS, 100, 1, true);
				...
				if(renderPartialTicksCheck > 0) {
					...
					if(renderPartialTicksCheck <= 0)
						Minecraft.getMinecraft().theWorld.playSound(player.posX, player.posY, player.posZ, SoundEvents.ENTITY_EXPERIENCE_ORB_PICKUP, SoundCategory.PLAYERS, 1F, 2F, false);
				}
				...
				if(isNightVisionActive && (player.inventory.armorInventory[3]==null || !(player.inventory.armorInventory[3].getItem() instanceof ItemComputerArmor))) {
					...
					Minecraft.getMinecraft().thePlayer.removePotionEffect(MobEffects.NIGHT_VISION);
				}
				...
				if(ClientProxy.kbArmorBoost.isKeyDown() && !isSprintKeyDown) {
					...
					if(((ItemComputerArmor)ItemsCore.computer_helmet).hasFullset(player) && renderPartialTicksCheck <= 0) {
						...
						for(int i = 0; i < 10; ++i)
							player.worldObj.playSound(player.posX, player.posY, player.posZ, SoundEvents.ENTITY_FIREWORK_LARGE_BLAST, SoundCategory.PLAYERS, 1, 0.01F+player.worldObj.rand.nextFloat(), false);
				...
				if(!player.capabilities.isFlying && player.isInWater() && ItemComputerArmor.class.cast(ItemsCore.computer_helmet).hasFullset(player) && Minecraft.getMinecraft().gameSettings.keyBindForward.isKeyDown()) {
					...
					for(int i = 0; i < 10; ++i)
						player.worldObj.spawnParticle(EnumParticleTypes.WATER_BUBBLE, d8, player.posY - 0.125D, d9, player.motionX, player.motionY, player.motionZ);
				}
			}
	}
	...
	public void pre(RenderGameOverlayEvent.Pre event) {
		try {
			if(event.getType() == ElementType.CROSSHAIRS) {
				if(ItemComputerArmor.hasFullset(Minecraft.getMinecraft().thePlayer)) {
					...
					GlStateManager.color(0, 1, 1, MathHelper.clamp_float(renderTime/20,0.1F,0.8F));
					...
					GlStateManager.color(1, 1, 1, MathHelper.clamp_float(renderTime/20,0.1F,0.8F));
				...
				if(Minecraft.getMinecraft().thePlayer.inventory.armorInventory[3] != null && Minecraft.getMinecraft().thePlayer.inventory.armorInventory[3].getItem() instanceof ItemComputerArmor) {
					...
					GlStateManager.color(0, 0, cB, MathHelper.clamp_float(renderTime/20,0.1F,0.8F));
			...
			if(event.getType() == RenderGameOverlayEvent.ElementType.HOTBAR) {
				...
				if(target != null && target.typeOfHit == RayTraceResult.Type.BLOCK) {
					...
					Block b = p.worldObj.getBlockState(target.getBlockPos()).getBlock();
					if(b != null && b instanceof BlockWindRune) {
						TileWindRune rune = (TileWindRune)p.worldObj.getTileEntity(target.getBlockPos());
						if(rune != null) {

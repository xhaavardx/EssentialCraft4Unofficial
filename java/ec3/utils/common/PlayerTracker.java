...
public class PlayerTracker{
	...
	{
		if(event.getEntityLiving() instanceof EntityPlayer && !event.getEntityLiving().worldObj.isRemote && event.getEntityLiving().worldObj.rand.nextFloat() < 0.0003F)
		{
			...
			if(ECUtils.getData(player).isWindbound())
			{
				...
				player.addChatMessage(new TextComponentString(TextFormatting.DARK_AQUA+""+TextFormatting.ITALIC+"The wind pushes you upwards..."));
	...
	public void onPlayerHurt(LivingHurtEvent event)
	{
		if(event.getEntityLiving() instanceof EntityPlayer && !event.getEntityLiving().worldObj.isRemote)
		{
			...
			if(chestplate != null && chestplate.getItem() == ItemsCore.magicArmorItems[5] && event.getEntityLiving().worldObj.rand.nextFloat() <= 0.2F)
			{
				event.setAmount(0F);
				for(int i = 0; i < 100; ++i)
				{
					MiscUtils.spawnParticlesOnServer("redstone", (float)(player.posX+MathUtils.randomDouble(event.getEntityLiving().worldObj.rand)/2), (float)(player.posY+1+MathUtils.randomDouble(event.getEntityLiving().worldObj.rand)), (float)(player.posZ+MathUtils.randomDouble(event.getEntityLiving().worldObj.rand)/2), -1, 0, 0);
				}
				return;
			}

...
public class PortalGeneratorFirstWorld extends DummyPortalGenerator {

	public static final PortalGeneratorFirstWorld INSTANCE = new PortalGeneratorFirstWorld();
	...
	public boolean makePortal(WorldServer worldIn, Entity entityIn, double x, double y, double z, Random random,
		...
		if(d0 < 0.0D) {
			j1 = MathHelper.clamp_int(j1, 70, worldIn.getActualHeight() - 10);
			k2 = j1;
		...
		for(int i8 = 0; i8 < 4; ++i8) {
			...
			for(int i9 = 0; i9 < 4; ++i9) {
				for(int i10 = -1; i10 < 4; ++i10) {
					...
					worldIn.notifyNeighborsOfStateChange(blockpos, worldIn.getBlockState(blockpos).getBlock());
				}
			}
		}

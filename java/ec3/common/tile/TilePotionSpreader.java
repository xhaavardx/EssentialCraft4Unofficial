	...
	public void update() {
		...
		if(worldObj.isBlockIndirectlyGettingPowered(pos) == 0) {
			if(potionID == -1)
				...
				List<EntityLivingBase> lst = worldObj.getEntitiesWithinAABB(EntityLivingBase.class, new AxisAlignedBB(pos.getX()-8, pos.getY()-8, pos.getZ()-8, pos.getX()+9, pos.getY()+9, pos.getZ()+9));
					...
					for(int i = 0; i < lst.size(); ++i) {
						...
						if(shouldUsePotion && getMRU() >= mruUsage) {
					        ...
					        for(int i1 = 0; i1 < 100; ++i1)
					        	MiscUtils.spawnParticlesOnServer("spell_mob", (float)(base.posX + MathUtils.randomFloat(worldObj.rand)), (float)(base.posY+1 + MathUtils.randomFloat(worldObj.rand)), (float)(base.posZ + MathUtils.randomFloat(worldObj.rand)), f, f1, f2);
						}
				...
				if(potionID != -1) {
					...
					f1 += (float)(j >> 8 & 255) / 255.0F;
					f2 += (float)(j >> 0 & 255) / 255.0F;
					worldObj.spawnParticle(EnumParticleTypes.SPELL_MOB, pos.getX()+0.5F, pos.getY()+0.5F, pos.getZ()+0.5F, f, f1, f2);
				}
			}
		}
	}

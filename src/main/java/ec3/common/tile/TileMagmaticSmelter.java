		...
		if(getWorld().isBlockIndirectlyGettingPowered(pos) == 0) {
			...
			if(ore != null && !getWorld().isRemote) {
				...
				if(metadata != -1) {
					...
					else if(getStackInSlot(4).getItem() == ItemsCore.magicalAlloy && OreSmeltingRecipe.getIndex(getStackInSlot(4)) == metadata && getStackInSlot(4).stackSize+2 <= getStackInSlot(4).getMaxStackSize() && getStackInSlot(4).stackSize + 4 <= getInventoryStackLimit())
					{
						if(getMRU() >= mruUsage && lavaTank != null && lavaTank.getFluid() != null && lavaTank.getFluid().getFluid() == FluidRegistry.LAVA && lavaTank.getFluidAmount() > 0)
						{
							setMRU(getMRU() - mruUsage);

	...
	public boolean generate(World w, Random r,BlockPos p)
	{
		...
		for(int dx = 0; dx < 3; ++dx)
		{
			for(int dz = 0; dz < 3; ++dz)
			{
				...
				{
					BlockPos dp = p.add(dx,dy,dz);
					if(dy == 0)
					{
						if(dx == 1 && dz == 1)
						{
							w.setBlockState(new BlockPos(x+dx, y, z+dz), BlocksCore.magicPlating.getDefaultState(),2);
						...
						{
							w.setBlockState(new BlockPos(x+dx, y-1, z+dz), BlocksCore.fortifiedStone.getDefaultState(),2);
						}
					}else
					{
						if((dx == 0 || dx == 2) && (dz == 0 || dz == 2) && dy < 5)
						{
							w.setBlockState(dp, BlocksCore.fence[2].getDefaultState(),2);
						}else if(dx == 1 && dz == 1 && dy < 6)
						{
							w.setBlockState(dp, BlocksCore.fence[1].getDefaultState(),2);
						}else if(dx == 1 && dz == 1 && dy == 6)
						{
							w.setBlockState(dp, BlocksCore.spreader.getDefaultState(),2);
						}
					}
				}
			}

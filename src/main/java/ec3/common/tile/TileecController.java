...
public class TileecController extends TileEntity implements ITEHasMRU, ITickable {
	...
	public IMRUPressence getMRUCU() {
		if(isCorrect) {
			List<Entity> eList = getWorld().getEntitiesWithinAABB(Entity.class, new AxisAlignedBB(lowerCoord.x, lowerCoord.y, lowerCoord.z, upperCoord.x, upperCoord.y, upperCoord.z));
					...
					for(int z = minZ; z <= maxZ; ++z) {
						if(z == minZ || z == maxZ || x == minX || x == maxX || y == minY || y == maxY) {
							...
							if(allowedBlocks.contains(getWorld().getBlockState(cp).getBlock())) {
								...
								if(ECUtils.ignoreMeta.containsKey(getWorld().getBlockState(cp).getBlock().getUnlocalizedName()) && ECUtils.ignoreMeta.get(getWorld().getBlockState(cp).getBlock().getUnlocalizedName()))
									meta = -1;
								...
								if(getWorld().getTileEntity(cp) != null && getWorld().getTileEntity(cp) instanceof IStructurePiece) {
									...
									if(getWorld().getTileEntity(cp) instanceof TileecHoldingChamber)

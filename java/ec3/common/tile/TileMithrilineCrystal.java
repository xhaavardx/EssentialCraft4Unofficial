...
public class TileMithrilineCrystal extends TileEntity implements ITickable {
	...
	public void update() {
		...
		boolean hasSky = worldObj.canBlockSeeSky(pos.up(3)) || !requiresUnobstructedSky;
		if(hasSky) {
			float energyGenerated = worldObj.provider != null && worldObj.provider.getDimension() == 1 ? energyEachTick_End : energyEachTick;

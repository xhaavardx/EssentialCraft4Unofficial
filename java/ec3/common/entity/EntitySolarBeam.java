...
public class EntitySolarBeam extends EntityWeatherEffect {
	...
	public EntitySolarBeam(World p_i1703_1_, double p_i1703_2_, double p_i1703_4_, double p_i1703_6_)
    {
        ...
        if (!p_i1703_1_.isRemote && p_i1703_1_.getGameRules().getBoolean("doFireTick") && p_i1703_1_.isAreaLoaded(new BlockPos(MathHelper.floor_double(p_i1703_2_), MathHelper.floor_double(p_i1703_4_), MathHelper.floor_double(p_i1703_6_)), 10))
        {
    ...
    public void onUpdate()
    {
        ...
        if(--beamLiveTime <= 0)
        	this.setDead();
        if((beamLiveTime%5) == 0)
        	this.worldObj.playSound(null, this.posX, this.posY, this.posZ, SoundEvents.BLOCK_FIRE_AMBIENT, SoundCategory.WEATHER, 10.0F, 2F);
        double d0 = 6.0D;
        List<?> list = this.worldObj.getEntitiesWithinAABBExcludingEntity(this, new AxisAlignedBB(this.posX - d0, this.posY - d0, this.posZ - d0, this.posX + d0, this.posY + 128.0D + d0, this.posZ + d0));

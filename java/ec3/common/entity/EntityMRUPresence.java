...
public class EntityMRUPresence extends EntityLivingBase implements IMRUPressence {
	...
	public void merge() {
		if(!this.isDead) {
			List<EntityMRUPresence> l = this.worldObj.getEntitiesWithinAABB(EntityMRUPresence.class, new AxisAlignedBB(posX-0.5D, posY-0.5D, posZ-0.5D, posX+0.5D, posY+0.5D, posZ+0.5D));
			if(!l.isEmpty()) {
	...
	public void onEntityUpdate() {
		...
		if(!this.worldObj.isRemote && !this.isDead) {
			if(tickTimer <= 0) {
				...
				vec.rotatePitch(this.worldObj.rand.nextFloat()*360);
				vec.rotateYaw(this.worldObj.rand.nextFloat()*360);
				vec.rotatePitch(this.worldObj.rand.nextFloat()*360);
				vec.rotateYaw(this.worldObj.rand.nextFloat()*360);
				if(!this.getFlag()) {
					for(int i = 0; i < mainMRUState; ++i) {
						...
						Block blk = this.worldObj.getBlockState(new BlockPos((int)(vc.xCoord+posX),(int)(vc.yCoord+posY),(int)(vc.zCoord+posZ))).getBlock();
						Block blk1 = this.worldObj.getBlockState(new BlockPos((int)(vc1.xCoord+posX),(int)(vc1.yCoord+posY),(int)(vc1.zCoord+posZ))).getBlock();
						int meta = blk1.getMetaFromState(this.worldObj.getBlockState(new BlockPos((int)(vc1.xCoord+posX),(int)(vc1.yCoord+posY),(int)(vc1.zCoord+posZ))));
						float resistance = 1F;
						...
						if(Config.isCorruptionAllowed) {
							if(!(blk1 instanceof BlockCorruption_Light) && !(blk instanceof BlockCorruption_Light) && blk1 != Blocks.AIR && blk == Blocks.AIR) {
								if(!this.worldObj.isRemote && this.worldObj.rand.nextInt((int) (1000*resistance)) <= mainMRUState) {
									this.worldObj.setBlockState(new BlockPos((int)(vc.xCoord+posX),(int)(vc.yCoord+posY),(int)(vc.zCoord+posZ)), id.getStateFromMeta(0), 3);
									break;
								}
							}
							if(blk instanceof BlockCorruption_Light) {
								int metadata = blk.getMetaFromState(this.worldObj.getBlockState(new BlockPos((int)(vc.xCoord+posX),(int)(vc.yCoord+posY),(int)(vc.zCoord+posZ))));
								if(metadata < 7 && this.worldObj.rand.nextInt((int) (1000*resistance)) <= mainMRUState)
								{
									this.worldObj.setBlockState(new BlockPos((int)(vc.xCoord+posX),(int)(vc.yCoord+posY),(int)(vc.zCoord+posZ)), blk.getStateFromMeta(metadata+1), 3);
								}
							}
						}
					}
					List<EntityPlayer> players = this.worldObj.getEntitiesWithinAABB(EntityPlayer.class, new AxisAlignedBB(posX-0.5D, posY-0.5D, posZ-0.5D, posX+0.5D, posY+0.5D, posZ+0.5D).expand(12, 12, 12));
					for(int i = 0; i < players.size(); ++i) {
						...
						for(double j = 0; j < playerCheck.lengthVector(); j += 0.5D) {
							...
							Block b = this.worldObj.getBlockState(new BlockPos(dX, dY, dZ)).getBlock();
							int meta = b.getMetaFromState(this.worldObj.getBlockState(new BlockPos(dX, dY, dZ)));
			...
			if(this.worldObj.rand.nextFloat() < 0.025F)
				this.worldObj.playSound(posX, posY, posZ, SoundRegistry.entityMRUCUNoise, SoundCategory.BLOCKS, (float)this.getMRU()/60000F, 0.1F+this.worldObj.rand.nextFloat(), false);
		}

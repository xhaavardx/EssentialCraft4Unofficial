...
public class TileMagicalFurnace extends TileMRUGeneric {
	...
	public boolean isStructureCorrect() {
		...
		for(int x = -2; x <= 2; ++x) {
			for(int z = -2; z <= 2; ++z) {
				flag = worldObj.getBlockState(pos.add(x, -1, z)).getBlock() == BlocksCore.voidStone;
				if(!flag)
					return false;
			}
		}
		flag = 
				...
				worldObj.getBlockState(pos.add(2, 0, -2)).getBlock() == BlocksCore.voidStone &&
				worldObj.getBlockState(pos.add(-2, 0, -2)).getBlock() == BlocksCore.voidStone &&
				worldObj.getBlockState(pos.add(2, 1, 2)).getBlock() == BlocksCore.heatGenerator &&
				worldObj.getBlockState(pos.add(-2, 1, 2)).getBlock() == BlocksCore.heatGenerator &&  
				worldObj.getBlockState(pos.add(2, 1, -2)).getBlock() == BlocksCore.heatGenerator &&
				worldObj.getBlockState(pos.add(-2, 1, -2)).getBlock() == BlocksCore.heatGenerator;
		return flag;
	...
	public void smelt() {
		...
		if(isStructureCorrect() && getMRU() > 0 && smeltingStack != null && smeltingItem != null) {
			if(progressLevel == 0)
				worldObj.playSound(null, pos, SoundEvents.BLOCK_FIRE_AMBIENT, SoundCategory.BLOCKS, 1.0F, 1);
			ItemStack mainSmelting = smeltingStack.copy();
			++progressLevel;
			setMRU(getMRU() - mruUsage);
			if(progressLevel >= smeltingTime) {
				progressLevel = 0;
				worldObj.playSound(null, pos, SoundEvents.BLOCK_LAVA_POP, SoundCategory.BLOCKS, 1.0F, 1);
				...
				if(!worldObj.isRemote && worldObj.rand.nextFloat() <= chanceToDoubleOutput) {
					EntityItem smelted = new EntityItem(worldObj, pos.getX()-1.5, pos.getY()+2.15, pos.getZ()-1.5, s.copy());
					worldObj.spawnEntityInWorld(smelted);
					smelted.motionX = 0;
					smelted.motionY = 0;
					smelted.motionZ = 0;
				}
				EntityItem smelted = new EntityItem(worldObj,pos.getX()+2.5, pos.getY()+2.15, pos.getZ()+2.5, s.copy());
				if(!worldObj.isRemote)
					worldObj.spawnEntityInWorld(smelted);
				smelted.motionX = 0;
				smelted.motionY = 0;
				...
				if(!worldObj.isRemote && worldObj.rand.nextFloat() <= chanceToDoubleSlags) {
					EntityItem slag = new EntityItem(worldObj,pos.getX()-1.5, pos.getY()+2.15, pos.getZ()+2.5, new ItemStack(ItemsCore.magicalSlag));
					worldObj.spawnEntityInWorld(slag);
					smelted.motionX = 0;
					smelted.motionY = 0;
					smelted.motionZ = 0;
				}
				EntityItem slag = new EntityItem(worldObj,pos.getX()+2.5,pos.getY()+2.15,pos.getZ()-1.5,new ItemStack(ItemsCore.magicalSlag));
				if(!worldObj.isRemote)
					worldObj.spawnEntityInWorld(slag);
				smelted.motionX = 0;
				smelted.motionY = 0;
	...
	public EntityItem getSmeltingItem() {
		EntityItem ret = null;
		List<EntityItem> l = worldObj.getEntitiesWithinAABB(EntityItem.class, new AxisAlignedBB(pos.getX(), pos.getY(), pos.getZ(), pos.getX()+1, pos.getY()+2, pos.getZ()+1));
		if(!l.isEmpty()) {

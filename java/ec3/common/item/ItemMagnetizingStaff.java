...
public class ItemMagnetizingStaff extends ItemStoresMRUInNBT implements IModelRegisterer {
	...
	public void onUsingTick(ItemStack stack, EntityLivingBase player, int count)
	{
		if(count % 8 == 0 && player instanceof EntityPlayer && (ECUtils.tryToDecreaseMRUInStorage((EntityPlayer)player, -50) || this.setMRU(stack, -50)))
		{
			...
			List<EntityItem> items = player.worldObj.getEntitiesWithinAABB(EntityItem.class, new AxisAlignedBB(player.posX-0.5D, player.posY, player.posZ-0.5D, player.posX+0.5D, player.posY+1, player.posZ+0.5D).expand(12D, 6D, 12D));
			...
			List<EntityXPOrb> orbs = player.worldObj.getEntitiesWithinAABB(EntityXPOrb.class, new AxisAlignedBB(player.posX-0.5D, player.posY, player.posZ-0.5D, player.posX+0.5D, player.posY+1, player.posZ+0.5D).expand(12D, 6D, 12D));

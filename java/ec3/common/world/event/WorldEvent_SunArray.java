...
public class WorldEvent_SunArray implements IWorldEvent{
	...
	public void playerTick(EntityPlayer p, int leftoverTime) {
		...
		if(!p.capabilities.isCreativeMode && p.dimension == Config.dimensionID && p.worldObj.canBlockSeeSky(new BlockPos(MathHelper.floor_double(p.posX), MathHelper.floor_double(p.posY+2), MathHelper.floor_double(p.posZ))) && !ignoreSun)
		{
		...
		{
			EntityFallingBlock sand = new EntityFallingBlock(p.worldObj, Math.floor(p.posX+MathUtils.randomDouble(p.worldObj.rand)*128)+0.5D, 255, Math.floor(p.posZ+MathUtils.randomDouble(p.worldObj.rand)*128)+0.5D, Blocks.FIRE.getDefaultState());
			sand.fallTime = 3;

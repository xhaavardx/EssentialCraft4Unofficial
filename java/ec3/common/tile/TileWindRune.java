...
public class TileWindRune extends TileEntity implements ITickable {
							...
							if(cenergy >= energy) {
								...
								if(rec.transforming.getItem() instanceof ItemSoulStone && !ECUtils.getData(player).isWindbound()) {
									...
									if(!player.worldObj.isRemote) {
										player.addChatMessage(new TextComponentString(I18n.translateToLocal("ec3.txt.windImbue")).setStyle(new Style().setColor(TextFormatting.AQUA)));
								...
								for(int j = 0; j < 300; ++j) 
									EssentialCraftCore.proxy.SmokeFX(pos.getX()+0.5D + MathUtils.randomDouble(worldObj.rand)*1.6D, pos.getY(), pos.getZ()+0.5D + MathUtils.randomDouble(worldObj.rand)*1.6D, 0, worldObj.rand.nextDouble()*0.3D, 0, 1, 0.7D, 1.0D, 0.85D);
	...
	public void update() {
		...
		if(tier >= 0) {
			EssentialCraftCore.proxy.SmokeFX(pos.getX()+0.5D + MathUtils.randomDouble(worldObj.rand)*1.6D, pos.getY(), pos.getZ()+0.5D + MathUtils.randomDouble(worldObj.rand)*1.6D, 0, worldObj.rand.nextDouble()*0.3D, 0, 1, 0.2D, 1.0D, 0.45D);

...
public class BlockRedstoneDeviceNotSided extends BlockContainer implements IModelRegisterer {
	...
	{
		if(e.isShearable(new ItemStack(Items.SHEARS), en.worldObj, en.getPosition()))
		{
			List<ItemStack> items = e.onSheared(new ItemStack(Items.SHEARS), en.worldObj, en.getPosition(), 2);
	...
	public void plant(EntityItem e)
	{
		...
		if(e.getEntityItem() != null && !e.worldObj.isRemote)
		{
			...
			if(stk.getItem() instanceof ItemBlock)
			{
				...
				{
					if(e.worldObj.isAirBlock(p) && e.worldObj.getBlockState(p.down()).getBlock().canSustainPlant(e.worldObj.getBlockState(p), e.worldObj, p, EnumFacing.UP, IPlantable.class.cast(b)))
					{
						FakePlayer user = new FakePlayer((WorldServer)e.worldObj,planterFakePlayerProfile);

						stk.getItem().onItemUse(stk, user, e.worldObj, p.down(), EnumHand.MAIN_HAND, EnumFacing.UP, 0, 0, 0);
			...
			}else //Items - seeds, reeds, cake?
			{
				...
				{
					if(e.worldObj.isAirBlock(p) && e.worldObj.getBlockState(p.down()).getBlock().canSustainPlant(e.worldObj.getBlockState(p), e.worldObj, p, EnumFacing.UP, IPlantable.class.cast(stk.getItem())))
					{
						FakePlayer user = new FakePlayer((WorldServer) e.worldObj,planterFakePlayerProfile);

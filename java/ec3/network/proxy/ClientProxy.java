...
public class ClientProxy extends CommonProxy {
	...
	public void spawnParticle(String name, float x, float y, float z, double i, double j, double k) {
		if(true && doParticle()) {
			if(name.equals("mruFX"))
				Minecraft.getMinecraft().effectRenderer.addEffect(new MRUFX(Minecraft.getMinecraft().theWorld, x, y, z, i, j, k));
			if(name.equals("cSpellFX"))
				Minecraft.getMinecraft().effectRenderer.addEffect(new CSpellFX(Minecraft.getMinecraft().theWorld, x, y, z, i, j, k));
			if(name.equals("fogFX"))
				Minecraft.getMinecraft().effectRenderer.addEffect(new FogFX(Minecraft.getMinecraft().theWorld, x, y, z, i, j, k));
		}
	}
		...
		try {
			if(stk.getItem() instanceof ItemSecret) {
				...
				case 0:
					...
					Biome biome = wrld.getBiomeGenForCoords(player.getPosition());
	...
	public void ItemFX(double... ds) {
		if(true && doParticle()) {
			if(ds.length == 6)
				Minecraft.getMinecraft().effectRenderer.addEffect(new ItemFX(Minecraft.getMinecraft().theWorld, ds[0], ds[1], ds[2], 1, 0, 1, ds[3], ds[4], ds[5]));
		}
	}
		...
		if(true && doParticle()) {
			if(ds.length == 10)
				Minecraft.getMinecraft().effectRenderer.addEffect(
						new ColoredFlameFX(Minecraft.getMinecraft().theWorld, ds[0], ds[1], ds[2], ds[3], ds[4], ds[5], ds[6], ds[7], ds[8], ds[9]));
		}
	}
	...
	public void SmokeFX(double... ds) {
		if(true && doParticle()) {
			if(ds.length == 7)
				Minecraft.getMinecraft().effectRenderer.addEffect(new SmokeFX(Minecraft.getMinecraft().theWorld, ds[0], ds[1], ds[2], ds[3], ds[4], ds[5], (float)ds[6]));
			if(ds.length == 10)
				Minecraft.getMinecraft().effectRenderer.addEffect(new SmokeFX(Minecraft.getMinecraft().theWorld, ds[0], ds[1], ds[2], ds[3], ds[4], ds[5], (float)ds[6], ds[7], ds[8], ds[9]));
		}
	}
	...
	public void MRUFX(double... ds) {
		if(true && doParticle()) {
			if(ds.length == 6)
				Minecraft.getMinecraft().effectRenderer.addEffect(new MRUFX(Minecraft.getMinecraft().theWorld, ds[0], ds[1], ds[2], ds[3], ds[4], ds[5]));
			if(ds.length == 9)
				Minecraft.getMinecraft().effectRenderer.addEffect(new MRUFX(Minecraft.getMinecraft().theWorld, ds[0], ds[1], ds[2], ds[3], ds[4], ds[5], ds[6], ds[7], ds[8]));
		}
	}
	...
	public void wingsAction(EntityPlayer e, ItemStack s)
	{
		...
		{
			e.worldObj.spawnParticle(EnumParticleTypes.REDSTONE, e.posX+MathUtils.randomDouble(e.worldObj.rand)/2, e.posY-1+MathUtils.randomDouble(e.worldObj.rand), e.posZ+MathUtils.randomDouble(e.worldObj.rand)/2, 0, 1, 1);
			e.motionY += 0.1F;
			e.fallDistance = 0F;
			double pX = e.posX;
			double pY = e.posY;
	...
	public void handleSoundPlay(DummyData[] packetData)
	{
		...
		EntityPlayer player = Minecraft.getMinecraft().thePlayer;
		player.worldObj.playSound(sX, sY, sZ, SoundEvent.REGISTRY.getObject(new ResourceLocation(sound)), SoundCategory.MASTER, volume, pitch, false);
	}

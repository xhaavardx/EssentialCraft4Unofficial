...
public class ItemMagicalDigger extends ItemPickaxe implements IItemRequiresMRU, IModelRegisterer {
		...
		for(int x = -1; x <= 1; ++x)
		{
			for(int y = -1; y <= 1; ++y)
			{
				for(int z = -1; z <= 1; ++z)
				{
					...
					for(int v = 0; v < 10; ++v)
						e.worldObj.spawnParticle(EnumParticleTypes.REDSTONE, c.x+x+e.worldObj.rand.nextFloat(),c.y+y+e.worldObj.rand.nextFloat(),c.z+z+e.worldObj.rand.nextFloat(), 1.0D, 0.0D, 1.0D);
					e.worldObj.playSound(e, e.getPosition(), SoundEvents.BLOCK_FIRE_EXTINGUISH, SoundCategory.BLOCKS, 0.2F, 6.0F);
					Block b = e.worldObj.getBlockState(new BlockPos((int)c.x+x,(int)c.y+y,(int)c.z+z)).getBlock();
					if(b != null && b == id)
					{
	...
	public void breakBlock(EntityPlayer e, Coord3D coord, ItemStack s)
	{
		...
		if(this.canBreak(s))
		{
			Block b = e.worldObj.getBlockState(p).getBlock();
			...
			int be = ForgeHooks.onBlockBreakEvent(e.worldObj, type, (EntityPlayerMP)e, p);
			if(be != -1)
			{
				b.harvestBlock(e.worldObj, e, p, e.worldObj.getBlockState(p), e.worldObj.getTileEntity(p), s);
				e.worldObj.setBlockToAir(p);
			}
		}
	}

		...
		if(worldObj.isBlockIndirectlyGettingPowered(pos) == 0) {
			if(!worldObj.isRemote) {
				if(burnedFlower == null && getMRU() < getMaxMRU()) {
					...
					Block b = worldObj.getBlockState(pos.add(offsetX, 0, offsetZ)).getBlock();
					...
					if(burnedFlower != null) {
						...
						if(burnTime <= 0) {
							if(createDeadBush)
								worldObj.setBlockState(new BlockPos((int)burnedFlower.x, (int)burnedFlower.y, (int)burnedFlower.z), Blocks.DEADBUSH.getStateFromMeta(0), 3);
							else
								worldObj.setBlockState(new BlockPos((int)burnedFlower.x, (int)burnedFlower.y, (int)burnedFlower.z), Blocks.AIR.getDefaultState(), 3);
							burnedFlower = null;
						}
					}
				}
			...
			if(burnedFlower != null && burnTime > 0) {
				worldObj.spawnParticle(EnumParticleTypes.FLAME, burnedFlower.x+0.5F + MathUtils.randomFloat(worldObj.rand)*0.3F, burnedFlower.y+0.1F + worldObj.rand.nextFloat()/2, burnedFlower.z+0.5F + MathUtils.randomFloat(worldObj.rand)*0.3F, 0, 0, 0);
				--burnTime;
				for(int t = 0; t < 200; ++t)
